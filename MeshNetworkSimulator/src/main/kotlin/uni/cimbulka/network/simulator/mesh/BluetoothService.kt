...
import uni.cimbulka.network.simulator.bluetooth.AdapterPool.lock
...
class BluetoothService(val adapter: BluetoothAdapter, name: String, private val simulator: AbstractSimulator) : CommService(name) {
    ...
    override val neighbors: List<Device>
        get() = AdapterPool.lock.withLock {
            val result = mutableListOf<Device>()
            btNeighbors.forEach {
                result.add(it.device)
            }
            return result
        }
    ...
    private val btNeighbors: MutableList<NetworkNode> = mutableListOf()
